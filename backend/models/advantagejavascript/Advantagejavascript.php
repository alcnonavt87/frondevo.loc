<?php
namespace backend\models\advantagejavascript;

use yii\base\Model;
use Yii;
use backend\models\AdminOthers;

Class Advantagejavascript extends Model
{
	var $frontendPath;
	var $myOthers;
	...
	public function getMany($filters, $limit, $offset, $lang) {
		...
		FROM
			`advantagejavascript`, `advantagejavascript_content`
		WHERE
			TRUE/*getManyFilterWhereClause*/ AND
			`idAdvantagejavascript` = `id` AND
			`lang` = :lang
		ORDER BY
			`id` ASC'
		.$limit
	...
	public function upDateProperty($id, $name, $value) {
        $query = Yii::$app->db->createCommand()->update('advantagejavascript', [$name => $value], 'id = :id', [':id' => $id]);
	...
	public function add($postBase, $postContent, $lang) {
		...
		if ($result) {
			...
			$query = Yii::$app->db->createCommand('INSERT INTO
				`advantagejavascript_content`
					(`idAdvantagejavascript`, `lang`'.$fields.')
			VALUES
				(:idAdvantagejavascript, :lang'.$values.')')
			->bindValue(':idAdvantagejavascript', $rowId)
			->bindValue(':lang', $lang)
			->execute();
			...
			$query = Yii::$app->db->createCommand('INSERT INTO
				`advantagejavascript_content`
					(`idAdvantagejavascript`, `lang`)
			VALUES
				(:idAdvantagejavascript, "'.$values[0].'"),
				(:idAdvantagejavascript, "'.$values[1].'")')
			->bindValue(':idAdvantagejavascript', $rowId)
			->execute();
		}
	...
	public function delete($id) {
		...
		if ($imagesOne) {
			...
			foreach ($fileNames[0] as $item) {
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantagejavascript/original-'.$item;
				$this->myOthers->deleteImgFromDisk($filePath);
				
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantagejavascript/medium-'.$item;
				$this->myOthers->deleteImgFromDisk($filePath);
			}
		}
		...
		foreach ($imagesMany as $item) {
			$query = Yii::$app->db->createCommand('SELECT
				`img`
			FROM
				`advantagejavascript_'.$item.'`
			WHERE
				`idAdvantagejavascript` = :id')
			->bindValue(':id', $id);
			
			$fileNames = $query->queryAll();
			
			foreach ($fileNames as $item) {
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantagejavascript/original-'.$item['img'];
				$this->myOthers->deleteImgFromDisk($filePath);
				
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantagejavascript/medium-'.$item['img'];
				$this->myOthers->deleteImgFromDisk($filePath);
			}
		}
