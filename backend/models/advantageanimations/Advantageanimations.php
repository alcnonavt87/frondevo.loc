<?php
namespace backend\models\advantageanimations;

use yii\base\Model;
use Yii;
use backend\models\AdminOthers;

Class Advantageanimations extends Model
{
	var $frontendPath;
	var $myOthers;
	...
	public function getMany($filters, $limit, $offset, $lang) {
		...
		FROM
			`advantageanimations`, `advantageanimations_content`
		WHERE
			TRUE/*getManyFilterWhereClause*/ AND
			`idAdvantageanimations` = `id` AND
			`lang` = :lang
		ORDER BY
			`id` ASC'
		.$limit
	...
	public function upDateProperty($id, $name, $value) {
        $query = Yii::$app->db->createCommand()->update('advantageanimations', [$name => $value], 'id = :id', [':id' => $id]);
	...
	public function add($postBase, $postContent, $lang) {
		...
		if ($result) {
			...
			$query = Yii::$app->db->createCommand('INSERT INTO
				`advantageanimations_content`
					(`idAdvantageanimations`, `lang`'.$fields.')
			VALUES
				(:idAdvantageanimations, :lang'.$values.')')
			->bindValue(':idAdvantageanimations', $rowId)
			->bindValue(':lang', $lang)
			->execute();
			...
			$query = Yii::$app->db->createCommand('INSERT INTO
				`advantageanimations_content`
					(`idAdvantageanimations`, `lang`)
			VALUES
				(:idAdvantageanimations, "'.$values[0].'"),
				(:idAdvantageanimations, "'.$values[1].'")')
			->bindValue(':idAdvantageanimations', $rowId)
			->execute();
		}
	...
	public function delete($id) {
		...
		if ($imagesOne) {
			...
			foreach ($fileNames[0] as $item) {
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantageanimations/original-'.$item;
				$this->myOthers->deleteImgFromDisk($filePath);
				
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantageanimations/medium-'.$item;
				$this->myOthers->deleteImgFromDisk($filePath);
			}
		}
		...
		foreach ($imagesMany as $item) {
			$query = Yii::$app->db->createCommand('SELECT
				`img`
			FROM
				`advantageanimations_'.$item.'`
			WHERE
				`idAdvantageanimations` = :id')
			->bindValue(':id', $id);
			
			$fileNames = $query->queryAll();
			
			foreach ($fileNames as $item) {
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantageanimations/original-'.$item['img'];
				$this->myOthers->deleteImgFromDisk($filePath);
				
				$filePath = $_SERVER['DOCUMENT_ROOT'].$this->frontendPath.'p/advantageanimations/medium-'.$item['img'];
				$this->myOthers->deleteImgFromDisk($filePath);
			}
		}
