...
class ListController extends \backend\controllers\AdminController
{
	public function actionIndex()
	{
        ...
        if ($isAjax) {
			...
			// Получаем записи
			$advantagejavascript = $myAdvantagejavascript->getMany($filters, $limit, $offset, $pageLang);//echo '<pre>';print_r($advantagejavascript);echo '</pre>';exit;


			$content = '';

            if (is_file(Yii::$app->basePath.'/views/pages/AdvantagejavascriptListView.php')) {
                require Yii::$app->basePath.'/views/pages/AdvantagejavascriptListView.php';
            }
