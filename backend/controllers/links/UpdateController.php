	...
	public function actionIndex()
	{
		...
		if ($isAjax) {//echo '<pre>';print_r($_POST);echo '</pre>';exit;
			...
			if (!$id2Uri && isset($_POST['list'])) {
				...
				$myOthers->sortTable('links', 'id', $newIndexes, $orderField);
				exit;
			}
			...
			if (!$newDoc) {
				// Обновляем запись
				$row = $myLinks->update($idRecord, $_POST['base'], $_POST['content'], $pageLang);
				...
				// Добавляем запись
				$row = $myLinks->add($_POST['base'], $_POST['content'], $pageLang);
			...
			foreach ($_SESSION['images'] as $item) { //только что добавленные
				...
				if ($uploader[0] == 'uploader0') { // одно изображение
					...
					$fileNameOriginal = "p/links/original-".$fileName;
					$fileNameMedium = "p/links/medium-".$fileName;
					...
					if ($_SERVER['REMOTE_ADDR'] == '127.0.0.1') {
						...
						$newRow = $myOthers->addImgOneMultiLangs('links', $uploader[1], $fileName, $imgTitle, $imgWidth, $imgHeight, $idRecord, 'idLinks', $pageLang);
					} else {
						/*// Загружаем как есть
						copy($tmp_dir.'/'.$name, $pathToFolder.$fileNameMedium);
						$newRow = $myOthers->addImgOne('links', $uploader[1], $fileName, $imgTitle, $imgWidth, $imgHeight, $idRecord);*/
						...
						$newRow = $myOthers->addImgOne('links', $uploader[1], $fileName, $imgTitle, 200, $h, $idRecord);*/
						...
						$newRow = $myOthers->addImgOne('links', $uploader[1], $fileName, $imgTitle, $w, 200, $idRecord);*/
						...
						$newRow = $myOthers->addImgOne('links', $uploader[1], $fileName, $imgTitle, $sizes[0], $sizes[1], $idRecord);*/
						...
						$newRow = $myOthers->addImgOne('links', $uploader[1], $fileName, $imgTitle, 200, 200, $idRecord);*/
					...
					if ($_SERVER['REMOTE_ADDR'] == '127.0.0.1') {
						copy($tmp_dir.'/'.$name, $pathToFolder.$fileNameMedium);
						$newRow = $myOthers->addImgManyMultiLangs('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, $imgWidth, $imgHeight, $pageLang);
					} else {
						/*// Загружаем как есть
						copy($tmp_dir.'/'.$name, $pathToFolder.$fileNameMedium);
						$newRow = $myOthers->addImgMany('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, $imgWidth, $imgHeight);*/
						
						/*// Создаём файл нужного размера по ширине
						$h = $myImagick->makeResizeImageByWidth(200, $fileNameMedium, $tmp_dir.'/'.$name, $format, imagick::FILTER_HAMMING, 0.8, 0, 1, imagick::COMPRESSION_LZW, 87);
						$newRow = $myOthers->addImgMany('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, 200, $h);*/
						
						/*// Создаём файл нужного размера по высоте
						$w = $myImagick->makeResizeImageByHeight(200, $fileNameMedium, $tmp_dir.'/'.$name, $format, imagick::FILTER_HAMMING, 0.8, 0, 1, imagick::COMPRESSION_LZW, 87);
						$newRow = $myOthers->addImgMany('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, $w, 200);*/
						
						/*// Создаём файл нужного размера по минимальной стороне
						$sizes = $myImagick->makeResizeImageByMinSide(200, 200, $fileNameMedium, $tmp_dir.'/'.$name, $format, imagick::FILTER_HAMMING, 0.8, 0, 1, imagick::COMPRESSION_LZW, 87);
						$newRow = $myOthers->addImgMany('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, $sizes[0], $sizes[1]);*/
						
						/*// Создаём файл нужного размера без обрезания
						$myImagick->makeResizeImage(200, 200, $fileNameMedium, $tmp_dir.'/'.$name, $format, imagick::FILTER_HAMMING, 0.8, 0, 1, imagick::COMPRESSION_LZW, 87);
						$newRow = $myOthers->addImgMany('links_'.$uploader[1], 'idLinks', $idRecord, $fileName, $imgTitle, 200, 200);*/
			...
			if (isset($images)) {
				foreach ($images as $key => $item) { //существующие, а также только что добавленные
					...
					if ($key[1] == 'one') { // одно изображение
						if (!$item['deleted']) {
							$myOthers->updateImgOneMultiLangs('links', $key[0], $item['imgTitle'], $idRecord, 'idLinks', $pageLang);
						} else {
							$myOthers->deleteImgOneMultiLangs('links', $key[0], $idRecord);
						}
					} else { // несколько изображений
						if (!$item['deleted']) {
							$myOthers->updateImgManyMultiLangs('links_'.$key[0], $item['imgTitle'], $item['picking'], $key[1], $pageLang);
						} else {
							$myOthers->deleteImgMany('links_'.$key[0], $key[1]);
						}
					}
				}
			}/* UpdateCodeBottom */
