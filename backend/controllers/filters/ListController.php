...
class ListController extends \backend\controllers\AdminController
{
	public function actionIndex()
	{
        ...
        if ($isAjax) {
			...
			// Получаем записи
			$filters = $myFilters->getMany($filters, $limit, $offset, $pageLang);//echo '<pre>';print_r($filters);echo '</pre>';exit;


			$content = '';

            if (is_file(Yii::$app->basePath.'/views/pages/FiltersListView.php')) {
                require Yii::$app->basePath.'/views/pages/FiltersListView.php';
            }
