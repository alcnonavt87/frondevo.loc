...
use Yii;
use backend\models\text\pages\Games;
use backend\models\AdminOthers;
use backend\models\Imagick;
use Yii\helpers\ArrayHelper;

class GamesupdateController extends \backend\controllers\AdminController {
        ...
        if (!$isAjax)
        {
			...
			foreach ($_SESSION['images'] as $item) { //только что добавленные
				...
				if ($uploader[0] == 'uploader0') { // одно изображение
					...
					$fileNameGeneral = Yii::$app->params['pics']['pages']['path']."biggamesbg-".$fileName;
					$imgGeneralWidth = Yii::$app->params['pics']['pages']['sizes']['imagegamesbgbig']['width'];
					$imgGeneralHeight = Yii::$app->params['pics']['pages']['sizes']['imagegamesbgbig']['height'];

					$fileNameSmall = Yii::$app->params['pics']['pages']['path']."smallgamesbg-".$fileName;
					$imgSmallWidth = Yii::$app->params['pics']['pages']['sizes']['imagegamesbgsmall']['width'];
					$imgSmallHeight = Yii::$app->params['pics']['pages']['sizes']['imagegamesbgsmall']['height'];


					if ($uploader[1] == 'imagegamesbgbig') {
					...
					} else if ($uploader[1] == 'imagegamesbgsmall') {
