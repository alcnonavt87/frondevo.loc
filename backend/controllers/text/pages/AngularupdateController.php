...
class AngularupdateController extends \backend\controllers\AdminController {
        ...
        if (!$isAjax)
        {
			...
			foreach ($_SESSION['images'] as $item) { //только что добавленные
				...
				if ($uploader[0] == 'uploader0') { // одно изображение
					...
					$fileNameGeneral = Yii::$app->params['pics']['pages']['path']."bigangularbg-".$fileName;
					$imgGeneralWidth = Yii::$app->params['pics']['pages']['sizes']['imageangularbgbig']['width'];
					$imgGeneralHeight = Yii::$app->params['pics']['pages']['sizes']['imageangularbgbig']['height'];

					$fileNameSmall = Yii::$app->params['pics']['pages']['path']."smallangularbg-".$fileName;
					$imgSmallWidth = Yii::$app->params['pics']['pages']['sizes']['imageangularbgsmall']['width'];
					$imgSmallHeight = Yii::$app->params['pics']['pages']['sizes']['imageangularbgsmall']['height'];

					if ($_SERVER['REMOTE_ADDR'] == '') {
						if ($uploader[1] == 'imageangularbgbig') {
						...
						} else if ($uploader[1] == 'imageangularsmall') {
						...
						if ($uploader[1] == 'imageangularbgbig') {
							$myImagick->makeResizeImageWithOptimalCrop($imgGeneralWidth, $imgGeneralHeight, $pathToFolder.$fileNameGeneral,$tmp_dir.'/'.$name, $format, \imagick::FILTER_UNDEFINED, 1, 0, 0, \imagick::COMPRESSION_LZW, 87);
							$newRow = $myOthers->addImgOneMultiLangsSBK('pages', $uploader[1], $fileName, $imgTitle, $idRecord, 'pageId', $pageLang,1);
						} else if ($uploader[1] == 'imageangularbgsmall') {
							$myImagick->makeResizeImageWithOptimalCrop($imgSmallWidth, $imgSmallHeight, $pathToFolder.$fileNameSmall,$tmp_dir.'/'.$name, $format, \imagick::FILTER_UNDEFINED, 1, 0, 0, \imagick::COMPRESSION_LZW, 87);
							$newRow = $myOthers->addImgOneMultiLangsSBK('pages', $uploader[1], $fileName, $imgTitle, $idRecord, 'pageId', $pageLang,1);
