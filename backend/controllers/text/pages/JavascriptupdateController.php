...
use Yii;
use backend\models\text\pages\Javascript;
use backend\models\AdminOthers;
use backend\models\Imagick;
use Yii\helpers\ArrayHelper;

class JavascriptupdateController extends \backend\controllers\AdminController {
        ...
        if (!$isAjax)
        {
			...
			foreach ($_SESSION['images'] as $item) { //только что добавленные
				...
				if ($uploader[0] == 'uploader0') { // одно изображение
					...
					$fileNameGeneral = Yii::$app->params['pics']['pages']['path']."bigjavascriptbg-".$fileName;
					$imgGeneralWidth = Yii::$app->params['pics']['pages']['sizes']['imagejavascriptbgbig']['width'];
					$imgGeneralHeight = Yii::$app->params['pics']['pages']['sizes']['imagejavascriptbgbig']['height'];

					$fileNameSmall = Yii::$app->params['pics']['pages']['path']."smalljavascriptbg-".$fileName;
					$imgSmallWidth = Yii::$app->params['pics']['pages']['sizes']['imagejavascriptbgsmall']['width'];
					$imgSmallHeight = Yii::$app->params['pics']['pages']['sizes']['imagejavascriptbgsmall']['height'];

					if ($_SERVER['REMOTE_ADDR'] == '') {
						if ($uploader[1] == 'imagejavascript5bgbig') {
						...
						} else if ($uploader[1] == 'imagejavascriptbgsmall') {
