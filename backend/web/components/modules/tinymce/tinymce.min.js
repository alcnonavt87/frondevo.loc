...
FRONDEVO_ADMIN.config.tinymce_init = function (object) {
    var adminPath = $('[data-admin-path]').eq(0).attr('data-admin-path') || '/adminfrondevo_/',
        path = adminPath + '';

    // decoration item ==============================
    tinymce.PluginManager.add('decoration', function(editor, url) {
        editor.addButton('decorationBtn', {
            text: false,
            icon: 'decoration',
            onclick: function(event) {
                var content = '<h2 class="decoration"><span data-mce-contenteditable="true">' + editor.selection.getContent() + '</span></h2>';
                editor.insertContent(content);
            }
        });
        editor.addMenuItem('decorationMenuItem', {
            text: 'Декор',
            context: 'context',
            icon: 'decoration',
            onclick: function() {
    ...
    tinymce.PluginManager.add('gallery', function(editor, url) {
        editor.addButton('galleryBtn', {
            text: 'G',
            onclick: function(event) {
                var content = '<span class="gallery"><span class="mceNonEditable" data-mce-contenteditable="true">|gallery|</span></span>';
                editor.insertContent(content);
            }
        });
        editor.addMenuItem('galleryMenuItem', {
            text: 'Галлерея',
            context: 'context',
            onclick: function() {
                var content = '<span class="gallery"><span class="mceNonEditable" data-mce-contenteditable="true">|gallery|</span></span>';
    ...
    var modules = FRONDEVO_ADMIN.modules,
        ...
        initPlugin = function () {
            tinymce.init({
                mode : "textareas",
                editor_selector : "text-area_tinymce",
                theme: "modern",
                language: 'ru',
                skin: 'motor',
                height: 300,
                ...
                plugins: [
                    "noneditable gallery decoration advlist autolink link image lists charmap print preview hr anchor pagebreak",
                    "searchreplace wordcount visualblocks visualchars insertdatetime media nonbreaking",
                    "table contextmenu directionality emoticons paste textcolor responsivefilemanager code fullscreen"
                ],
                toolbar1: "undo redo | bold italic underline | decorationBtn galleryBtn | visualblocks | cquote | alignleft aligncenter alignright alignjustify " +
                "| forecolor backcolor | bullist numlist outdent indent | code | fullscreen | preview",
                toolbar2: "link unlink anchor | image media",
                image_advtab: true,

                menu: {
                    edit: {title: 'Edit', items: 'undo redo | cut copy paste | selectall'},
                    insert: {title: 'Insert', items: 'image media | link anchor | charmap pagebreak nonbreaking hr ' +
                    '| insertdatetime'},
                    format: {title: 'Format', items: 'italic bold underline strikethrough superscript ' +
                    'subscript | decorationMenuItem galleryMenuItem | formats | removeformat'},
                    table: {title: 'Table'}
                },
                ...
                //external_filemanager_path: path + "components/modules/tinymce/plugins/filemanager/",
                external_filemanager_path: "/backend/web/components/modules/filemanager/",
                filemanager_title: "Менеджер файлов",
                external_plugins: {
                    //"filemanager" : "plugins/filemanager/plugin.min.js"
                    "filemanager" : "/backend/web/components/modules/filemanager/plugin.min.js"
                },
                extended_valid_elements : "hr[class|width|size|noshade],font[face|size|color|style],span[data-mce-type|class|align|style],noindex,script,object",
                custom_elements: 'decorationMenuItem',
                content_css: [
                    path + 'components/modules/tinymce/skins/motor/add.css'
                ],
				relative_urls: false
            });
        };
    ...
    if (object === undefined) {
        object = document.querySelectorAll('[data-module="tinymce"]');
    }

    if (object.length !== undefined) {
        length = object.length;
        for (i = 0; i < length; i++) {
            id = 'text-area_' + parseInt(new Date().getTime() + i + Math.random(), 10);
            object[i].removeAttribute('data-module');
            object[i].setAttribute('id', id);
            $(object[i]).addClass('text-area_tinymce');
        }
        ...
        id = 'text-area_' + parseInt(new Date().getTime() + Math.random(), 10);
        object.removeAttribute('data-module');
        object.setAttribute('id', id);
        $(object).addClass('text-area_tinymce');
        initPlugin(id);
    }
