...
$countRecords = count($advantagegames);
for($i = 0; $i < $countRecords; $i++)
{
	$content .= '<tr data-id = "'.$advantagegames[$i]['id'].'/'.$pageLang.'">';
				$content .= '<!--<td>'.($i + 1).'</td>
				<td class="table__col-left"><b>'.$advantagegames[$i]['id'].'</b></td>--><!-- td -->
				<td class="table__col-left">'.$advantagegames[$i]['title'].'</td>
				<!-- visibilityCheckboxTd -->
				<td class="td__edit-cell2">
					<div class="btn__wrap-inline">';
						$content .= '<a title="Редактировать запись" href="/'.$idPageGroup.'/'.$advantagegames[$i]['id'].'/'.$pageLang.'" data-href="formedit" class="btn__edit catalogue-list_edit"></a>';
